...
% a) Unary predicate which always evalautes to True if female or male

sheep(X) :- female(X); male(X).

% b) Binary predicate describing father and mother relationships

father(X,Y) :- parent(X,Y), male(X).
mother(X,Y) :- parent(X,Y), female(X).

% c) Binary predicate describing ancestor relationship
